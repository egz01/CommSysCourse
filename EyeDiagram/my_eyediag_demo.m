...
a = 1/1.1;
ct = (1-a*abs(tp)).*(abs(tp) <= 1);
rt = (abs(tp) <= 1/a);
%p1=(1-a*abs(tp)).*((0.5*a*abs(tp))<1);
p1 = ct.*rt;
figure; subplot(311); stem(tp, ct); xlim([-1 1]); title("c(t)=(1-a|t|)"); subplot(312); stem(tp, rt); xlim([-1 1]); title("r(t)=0.5*a*abs(t)<1");
subplot(313); stem(tp, p1); xlim([-1 1]); title("p(t)=c(t)r(t)");
