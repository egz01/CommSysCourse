...
%components)
etas=[-2.5 -1 5];   %means of the Gaussian components
sigs=[0.5 2 1];     %variances of the Gaussian components
MM=length(etas);    %number of components
f=0*x;
for mm=1:MM
    eta=etas(mm);
    sig=sigs(mm);
    f=f+exp(-0.5*((x-eta)/sig).^2)/sqrt(sig);   %similarly scaled Gaussians
end
